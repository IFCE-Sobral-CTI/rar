...
function Index({ requirementTypes, count, page, termSearch, can }) {
    ...
    return (
        <>
            <AuthenticatedLayout
                titleChildren={'Gerenciamento de tipos de requerimentos'}
                breadcrumbs={[{ label: 'Tipos de Requerimentos', url: route('types.index') }]}
            >
                <div className="flex gap-2 md:flex-row md:gap-4">
                    {can.create && <Panel className={'inline-flex'}>
                        <Link href={route('types.create')} className="inline-flex items-center justify-between gap-2 px-3 py-2 font-light text-white transition bg-blue-500 border border-transparent rounded-md focus:ring hover:bg-blue-600 focus:ring-sky-300">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="w-5 h-5" viewBox="0 0 16 16">
                ...
                <Panel className="">
                    ...
                    <Pagination data={requirementTypes} count={count} />
                </Panel>
            </AuthenticatedLayout>
        </>
    )
