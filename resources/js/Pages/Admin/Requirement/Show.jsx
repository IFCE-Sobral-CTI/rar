...
function Show({ requirement, can }) {
    ...
    return (
        <>
            ...
            <AuthenticatedLayout titleChildren={'Detalhes do requerimentos'} breadcrumbs={[{ label: 'Requerimentos', url: route('requirements.index') }, { label: requirement.description, url: route('requirements.show', requirement.id) }]}>
                <Panel className={'flex flex-col gap-4'}>
                    <ShowField label={'Situação'} value={<Status value={requirement.status} />} />
                    <ShowField label={'Tipo'} value={requirement.requirement_type.description} />
                    <ShowField label={'Matricula'} value={requirement.enrollment.number} />
                    <ShowField label={'Discente'} value={requirement.enrollment.student.name} />
